...
本文件为 Codex (Codex.ai/code) 提供项目上下文。
...
| `.Codex/skills/` | 开发期 Agent 技能（辅助项目开发） |
| `.Codex/commands/` | 自定义斜杠命令 |
| `shared/types/` | 前后端共享 TypeScript 类型 |

## 关键陷阱

这是 Codex 最容易踩的坑，**每条都必须遵守**：
...
Phase 1 + 1.5 全部完成 ✅。Phase 2 (Agent + Skills) 待启动。推迟项：SSE 重连、编辑重发、键盘快捷键、API 单元测试。详见 [roadmap](.Codex/plan/roadmap.md)。

## 开发期技能

| 技能 | 文件 | 用途 |
|------|------|------|
| `/doc-consolidate` | `.Codex/skills/doc-consolidate/SKILL.md` | 文档沉淀 — 斜杠命令 `/doc-consolidate` 或自然语言触发，Agent 可自动识别 |
