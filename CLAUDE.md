...
| [cloudflare-worker-build-oom](docs/dev-log/2026-07-05-cloudflare-worker-build-oom.md) | **Cloudflare Worker 构建 OOM 修复** — 缺失 `nitro.preset` 导致 `node-server` 全量打包 node_modules（Nitro 端），叠加 Vite 客户端处理 mermaid（80MB）等重型依赖；`cloudflare-module` + 中等堆调整（6GB）双管齐下；附 Nuxt 双管线构建架构解析 |
