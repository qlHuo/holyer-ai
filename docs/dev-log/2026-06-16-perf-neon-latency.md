...
本地开发慢不影响上线。部署到 Cloudflare Pages 后，Worker 和 Neon 都在云端骨干网上，单次查询 5–15ms。并行化后 chat 接口 DB 开销可压在 20ms 以内，瓶颈完全转移到 LLM 首 token。
...
- [ADR-004: Cloudflare Pages 部署](../decisions/004-cloudflare-pages.md) — 生产部署架构，Worker ↔ Neon 通信路径
